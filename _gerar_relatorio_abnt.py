# -*- coding: utf-8 -*-
"""Gera o Relatorio de Estagio em formato ABNT (NBR 14724) a partir dos .md."""
import re, io, os
...
def parse_md(lines, doc, clean=False, head_as_bold=False):
    ...
    while i < n:
        ...
        if s.startswith('```'):
            ...
            continue
        if re.match(r'^(\*\*\*+|---+|___+)$', s):
            i += 1; continue  # regua: ignora (ABNT nao usa)
        if s.startswith('|') and i + 1 < n and is_sep(lines[i + 1]):
            ...
            continue
        if s.startswith('>'):
            if clean:  # notas editoriais — descarta
                while i < n and lines[i].strip().startswith('>'):
                    i += 1
                continue
            q = []
...
centered(doc, 'CURSO DE ENGENHARIA DE PRODUÇÃO')
blanks(doc, 6)
centered(doc, 'LEONARDO SÓGLIA', bold=True)
blanks(doc, 8)
centered(doc, 'RELATÓRIO DE ESTÁGIO SUPERVISIONADO OBRIGATÓRIO', bold=True, size=14)
blanks(doc, 1)
centered(doc, 'Estágio realizado na Pequenas Mordidas Alimentos Eireli (Doces Vó Nena)')
blanks(doc, 1)
_nota = centered(doc, '(versão preliminar — sujeita a alterações)', size=11)
_nota.runs[0].italic = True
blanks(doc, 8)
...
rn = nat.add_run('Relatório de Estágio Supervisionado Obrigatório apresentado ao '
                 'Curso de Engenharia de Produção da Universidade Federal de Campina '
                 'Grande como requisito parcial para aprovação na disciplina de '
                 'Estágio Supervisionado.')
rn.font.name = TNR; rn.font.size = Pt(12)
blanks(doc, 1)
for line in ('Orientador: Prof. Kegenaldo',
             'Supervisor: Eraldo — sócio-gerente'):
...
# IDENTIFICACAO (pre-textual)
doc.add_page_break()
parse_md(read('relatorio_estagio/01_identificacao.md'), doc, clean=True, head_as_bold=True)
...
BODY = ['relatorio_estagio/secoes/1_introducao.md',
        'relatorio_estagio/secoes/2_empresa.md',
        'relatorio_estagio/secoes/3_atividades.md',
        'relatorio_estagio/secoes/4_resultados.md',
        'relatorio_estagio/secoes/5_consideracoes.md',
        'relatorio_estagio/secoes/6_referencias.md',
        'relatorio_estagio/secoes/7_perguntas.md']
...
hs = [p.text for p in chk.paragraphs if p.style.name.startswith('Heading')]
