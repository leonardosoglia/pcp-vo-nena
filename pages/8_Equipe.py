    ...
    if not funcionarios:
        ...
        for dept in DEPARTAMENTOS_FUNCIONARIO:
            ...
            for f in por_dept[dept]:
                ...
                with col_acoes:
                    if ativo:
                        if st.button("Inativar", key=f"del_{f['id']}", width='stretch'):
                            excluir_funcionario(f["id"])
                            invalidar_equipe()
                            st.success(f"{f['nome']} inativado.")
                            st.rerun()
                    else:
                        if st.button("Reativar", key=f"act_{f['id']}", width='stretch'):
...
with tab_cap:
    ...
    if not funcs_ativos:
        st.warning("Cadastra funcionários na aba anterior antes de definir capacidades.")
        st.stop()
    ...
    if func_sel:
        ...
        if not caps:
            st.markdown(
                "<div class='didatica'>"
                "Sem capacidades cadastradas. Use 'Adicionar capacidade' acima."
                "</div>",
                unsafe_allow_html=True,
            )
        else:
...
with tab_pres:
    ...
    if not presencas:
        st.warning("Cadastra funcionários na aba 'Funcionários' antes.")
        st.stop()
