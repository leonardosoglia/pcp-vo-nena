...
- **Arquitetura PCP ↔ SIGE (14/06/2026):** definida com a Gestão — SIGE = sistema de registro do ciclo de materiais (NF-e XML → estoque → OP explode ficha → pré-reserva → baixa por lote → finaliza com rendimento); a **OP é a ponte**; a decisão de produção é **humana** (Gestão + planejamento), não de sistema; nosso PCP é **somente leitura** (exceção pendente: como a OP entra — manual vs API). Detalhes em **`docs/ARQUITETURA_SIGE.md`**; requisitos/perguntas à Gestão em `docs/requisitos_gestora_sige.md`.
- **SIGE tem módulo de PCP/OP NATIVO — confirmado 17/06/2026 (API + central de ajuda `ajuda.sigecloud.com.br`):** o ciclo de materiais inteiro (ficha técnica = "Produtos Compostos" → OP explode/calcula insumos → baixa de estoque **ao iniciar OU ao finalizar, configurável** → entrada do acabado + rendimento → lote/subordens/qualidade) roda **nativamente no SIGE**. Na conta da fábrica está **vazio** (não alimentado). **Decisão:** o ciclo roda no SIGE, **NÃO duplicar** no nosso PCP; nosso PCP fica nas pontas (apoio à decisão humana + folha + análise + reconciliação), OP é a ponte. **OP entra MANUAL p/ começar** (mantém read-only; a API `OrdensProducao/Cadastrar` permite automatizar depois, ramo isolado/desligado). Consequência: nosso **BOM** vira apoio de planejamento + fonte p/ popular o SIGE; nossa **auto-baixa** (Etapa E) vira estimativa (ou desliga). Raio-X completo (§6-bis) em **`docs/ARQUITETURA_SIGE.md`**.
